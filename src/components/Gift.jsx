...
const Gift = () => {
  return (
    <div className="w-full h-auto lg:h-[109px] pl-6 pr-6 flex justify-center gap-[10px]">
        <div className="relative flex flex-col lg:flex-row items-center justify-center w-[84.66%] lg:w-[68.3062%] pt-6 pb-6 pl-[10px] pr-[10px] bg-red-500 rounded-3xl">
        <div
            className="absolute inset-0 bg-center bg-no-repeat bg-cover opacity-20"
            style={{ backgroundImage: `url(${blackBlur})` }}
        />
        <div className="absolute left-0 hidden max-[1512px]:!hidden -translate-y-1/2 lg:block w-55 h-55 top-1/2">
            <img className="w-full h-full" src={gift} alt="" />
        </div>
        <div className="flex flex-col w-full lg:w-[68.164%] h-full gap-1 ml-0 max-[1512px]:!w-full max-[1512px]:!ml-0 lg:ml-22 z-2">
        ...
        <button className="absolute right-0 hidden pt-3 pb-3 pl-6 pr-6 font-bold translate-x-1/2 -translate-y-1/2 bg-white hover:bg-white-200 lg:block h-11 top-1/2 text-base-16 font-space-grotesk leading-1 text-black-500 rounded-xl">
            <a href="https://vouchers.funarenacheb.cz/c/darkovy-poukaz" target="_blank" rel="noopener noreferrer">Buy now</a>
        </button>
